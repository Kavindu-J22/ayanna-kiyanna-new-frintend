...
const CreateGradeFileDialog = ({ open, onClose, folderId, gradeCategory, onFileCreated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://ayanna-kiyanna-new-backend.onrender.com/api/grades/files',
        {
          ...formData,
      ...
      <DialogContent sx={{ pt: 2 }}>
        ...
        <Box sx={{ mt: 3 }}>
          ...
          <Paper
            ...
            onClick={() => document.getElementById(`grade-file-upload-${gradeCategory}`).click()}
          >
            <input
              id={`grade-file-upload-${gradeCategory}`}
              type="file"
              multiple
              accept="image/*,.pdf"
              style={{ display: 'none' }}
            ...
            <UploadIcon sx={{ fontSize: 48, color: currentConfig.color, mb: 1 }} />
            <Typography
              variant="body1"
              sx={{
                fontFamily: '"Noto Sans Sinhala", "Yaldevi", sans-serif',
          ...
          {uploading && (
            <Box display="flex" justifyContent="center" mt={2}>
              <CircularProgress size={30} sx={{ color: currentConfig.color }} />
            </Box>
          )}
        </Box>
...
export default CreateGradeFileDialog;
