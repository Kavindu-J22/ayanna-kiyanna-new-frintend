...
const ReviewsFileDetail = () => {
  ...
  const fetchFileDetails = async () => {
    try {
      ...
      // Fetch file details
      const fileResponse = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/files/${fileId}`, {
        headers: { 'x-auth-token': token }
      });
      setFile(fileResponse.data.data);

      // Fetch comments
      const commentsResponse = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/files/${fileId}/comments`, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleLike = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/files/${fileId}/like`, {}, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleAddComment = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/files/${fileId}/comments`, {
        content: newComment
      }, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleEditComment = async (commentId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/comments/${commentId}`, {
        content: editText
      }, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleDeleteComment = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/comments/${commentToDelete}`, {
        headers: { 'x-auth-token': token }
      });
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Breadcrumbs sx={{ mb: 3 }}>
        <Link
          ...
          variant="body1"
          onClick={() => navigate('/')}
          sx={{ display: 'flex', alignItems: 'center', color: '#e91e63' }}
        >
        ...
        <Link
          component="button"
          variant="body1"
          onClick={() => navigate(`/reviews/folder/${file?.folderId._id}`)}
          sx={{ color: '#e91e63' }}
        >
          {file?.folderId.title}
        </Link>
      ...
      {/* File Content */}
      <Card sx={{ mb: 4, borderRadius: 3, boxShadow: '0 8px 32px rgba(233, 30, 99, 0.1)' }}>
        <CardContent sx={{ p: 4 }}>
          {/* File Header */}
          <Box display="flex" alignItems="center" mb={3}>
            <Avatar sx={{ bgcolor: '#e91e63', mr: 2, width: 56, height: 56 }}>
              <CommentIcon />
            </Avatar>
            <Box flexGrow={1}>
                ...
                {file.attachments.map((attachment, index) => (
                  <Grid item xs={12} sm={6} md={4} key={index}>
                    <Card sx={{ 
                      ...
                      '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 8px 16px rgba(233, 30, 99, 0.2)' }
                    }}>
                      <CardContent sx={{ p: 2 }}>
                ...
                {file.sourceLinks.map((link, index) => (
                  <Grid item xs={12} sm={6} key={index}>
                    <Card sx={{ 
                      ...
                      borderRadius: 2,
                      transition: 'all 0.3s ease',
                      '&:hover': { borderColor: '#c2185b', boxShadow: '0 4px 8px rgba(233, 30, 99, 0.2)' }
                    }}>
                      <CardContent sx={{ p: 2 }}>
                        <Box display="flex" alignItems="center" mb={1}>
                          <LinkIcon sx={{ color: '#e91e63', mr: 1 }} />
                          <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>
                            {link.title}
                          </Typography>
                        </Box>
      ...
      {/* Comments Section */}
      <Card sx={{ borderRadius: 3, boxShadow: '0 8px 32px rgba(233, 30, 99, 0.1)' }}>
        <CardContent sx={{ p: 4 }}>
          <Typography variant="h5" sx={{ mb: 3, color: '#e91e63', fontWeight: 'bold' }}>
...
export default ReviewsFileDetail;
