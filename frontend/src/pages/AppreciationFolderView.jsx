...
const AppreciationFolderView = () => {
  ...
  const fetchFolderAndFiles = async () => {
    try {
      ...
      const folderResponse = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/folders/${folderId}`, {
        headers: { 'x-auth-token': token }
      });
      setFolder(folderResponse.data.data);

      // Fetch files in folder
      const filesResponse = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/folders/${folderId}/files`, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleCreateFile = async () => {
    try {
      ...
      await axios.post(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/files`, fileData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleEditFile = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/files/${selectedFile._id}`, formData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleDeleteFile = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/files/${selectedFile._id}`, {
        headers: { 'x-auth-token': token }
      });
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            >
              <CardContent sx={{ flexGrow: 1, p: 3 }}>
                ...
                <Box display="flex" alignItems="center" justifyContent="space-between">
                  <Box display="flex" alignItems="center" gap={1}>
                    <FavoriteIcon sx={{ color: '#9c27b0', fontSize: 18 }} />
              ...
              <CardActions sx={{ p: 2, pt: 0 }}>
                <Button
                  ...
                  onClick={() => navigate(`/appreciation/file/${file._id}`)}
                  sx={{
                    background: 'linear-gradient(45deg, #9c27b0, #e91e63)',
                    color: 'white',
                    fontWeight: 'bold',
      ...
      <Dialog open={openCreateDialog} onClose={() => setOpenCreateDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ bgcolor: '#9c27b0', color: 'white', fontWeight: 'bold' }}>
...
export default AppreciationFolderView;
