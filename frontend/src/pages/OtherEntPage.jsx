...
const OtherEntPage = () => {
  ...
  const fetchFolders = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/folders`, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleCreateFolder = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/folders`, formData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleEditFolder = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/folders/${selectedFolder._id}`, formData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleDeleteFolder = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/folders/${selectedFolder._id}`, {
        headers: { 'x-auth-token': token }
      });
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Box textAlign="center" sx={{ mb: 4 }}>
        ...
        >
          අමතර සාහිත්‍ය අන්තර්ගතය සහ විශේෂ ලිපි
        </Typography>
      </Box>
            ...
            >
              ...
              <CardActions sx={{ p: 2, pt: 0 }}>
                <Button
                  ...
                  onClick={() => navigate(`/otherEnt/folder/${folder._id}`)}
                  sx={{
                    background: 'linear-gradient(45deg, #ff5722, #ff9800)',
                    color: 'white',
                    fontWeight: 'bold',
