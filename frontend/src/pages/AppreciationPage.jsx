...
const AppreciationPage = () => {
  ...
  const fetchFolders = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/folders`, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleCreateFolder = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/folders`, formData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleEditFolder = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/folders/${selectedFolder._id}`, formData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleDeleteFolder = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://ayanna-kiyanna-new-backend.onrender.com/api/appreciation/folders/${selectedFolder._id}`, {
        headers: { 'x-auth-token': token }
      });
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Box textAlign="center" sx={{ mb: 4 }}>
        ...
        >
          සාහිත්‍ය රසවින්දන සහ අගය කිරීම සඳහා වන අන්තර්ගතය
        </Typography>
      </Box>
            ...
            >
              ...
              <CardActions sx={{ p: 2, pt: 0 }}>
                <Button
                  ...
                  onClick={() => navigate(`/appreciation/folder/${folder._id}`)}
                  sx={{
                    background: 'linear-gradient(45deg, #9c27b0, #e91e63)',
                    color: 'white',
                    fontWeight: 'bold',
