...
const OtherEntFileDetail = () => {
  ...
  const fetchFileDetails = async () => {
    try {
      ...
      // Fetch file details
      const fileResponse = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/files/${fileId}`, {
        headers: { 'x-auth-token': token }
      });
      setFile(fileResponse.data.data);

      // Fetch comments
      const commentsResponse = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/files/${fileId}/comments`, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleLike = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/files/${fileId}/like`, {}, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleAddComment = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/files/${fileId}/comments`, {
        content: newComment
      }, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleEditComment = async (commentId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/comments/${commentId}`, {
        content: editText
      }, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleDeleteComment = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://ayanna-kiyanna-new-backend.onrender.com/api/otherEnt/comments/${commentToDelete}`, {
        headers: { 'x-auth-token': token }
      });
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Breadcrumbs sx={{ mb: 3 }}>
        ...
        <Link
          component="button"
          variant="body1"
          onClick={() => navigate(`/otherEnt/folder/${file?.folderId._id}`)}
          sx={{ color: '#ff5722' }}
        >
          {file?.folderId.title}
        </Link>
      ...
      {/* File Content */}
      <Card sx={{ mb: 4, borderRadius: 3, boxShadow: '0 8px 32px rgba(255, 87, 34, 0.1)' }}>
        <CardContent sx={{ p: 4 }}>
          {/* File Header */}
          <Box display="flex" alignItems="center" mb={3}>
            <Avatar sx={{ bgcolor: '#ff5722', mr: 2, width: 56, height: 56 }}>
              <CommentIcon />
            </Avatar>
            <Box flexGrow={1}>
                ...
                {file.attachments.map((attachment, index) => (
                  <Grid item xs={12} sm={6} md={4} key={index}>
                    <Card sx={{ 
                      ...
                      '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 8px 16px rgba(255, 87, 34, 0.2)' }
                    }}>
                      <CardContent sx={{ p: 2 }}>
                ...
                {file.sourceLinks.map((link, index) => (
                  <Grid item xs={12} sm={6} key={index}>
                    <Card sx={{ 
                      ...
                      borderRadius: 2,
                      transition: 'all 0.3s ease',
                      '&:hover': { borderColor: '#f4511e', boxShadow: '0 4px 8px rgba(255, 87, 34, 0.2)' }
                    }}>
                      <CardContent sx={{ p: 2 }}>
                        <Box display="flex" alignItems="center" mb={1}>
                          <LinkIcon sx={{ color: '#ff5722', mr: 1 }} />
                          <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>
                            {link.title}
                          </Typography>
                        </Box>
                        ...
                        <Button
                          ...
                          sx={{
                            borderColor: '#ff5722',
                            color: '#ff5722',
                            '&:hover': { borderColor: '#f4511e', bgcolor: '#fff3e0' }
                          }}
                        >
                          සබැඳිය බලන්න
                        </Button>
      ...
      {/* Comments Section - Similar to others but with orange theme */}
      <Card sx={{ borderRadius: 3, boxShadow: '0 8px 32px rgba(255, 87, 34, 0.1)' }}>
        <CardContent sx={{ p: 4 }}>
...
export default OtherEntFileDetail;
