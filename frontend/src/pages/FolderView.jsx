...
const FolderView = () => {
  ...
  const fetchFolderAndFiles = async () => {
    try {
      ...
      const filesResponse = await axios.get(
        `https://ayanna-kiyanna-new-backend.onrender.com/api/grammar/folders/${folderId}/files`,
        { headers: { 'x-auth-token': token } }
      );
  ...
  const handleDeleteFile = async (fileId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(
        `https://ayanna-kiyanna-new-backend.onrender.com/api/grammar/files/${fileId}`,
        { headers: { 'x-auth-token': token } }
      );
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      {files.length === 0 && !loading && (
        ...
        >
          ...
          >
            <FileIcon sx={{ fontSize: 80, color: '#E91E63', mb: 2 }} />
            <Typography
              variant="h5"
              gutterBottom
              sx={{
