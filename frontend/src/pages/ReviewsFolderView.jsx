...
const ReviewsFolderView = () => {
  ...
  const fetchFolderAndFiles = async () => {
    try {
      ...
      const folderResponse = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/folders/${folderId}`, {
        headers: { 'x-auth-token': token }
      });
      setFolder(folderResponse.data.data);

      // Fetch files in folder
      const filesResponse = await axios.get(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/folders/${folderId}/files`, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleCreateFile = async () => {
    try {
      ...
      await axios.post(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/files`, fileData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleEditFile = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/files/${selectedFile._id}`, formData, {
        headers: { 'x-auth-token': token }
      });
  ...
  const handleDeleteFile = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://ayanna-kiyanna-new-backend.onrender.com/api/reviews/files/${selectedFile._id}`, {
        headers: { 'x-auth-token': token }
      });
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            >
              ...
              <CardActions sx={{ p: 2, pt: 0 }}>
                <Button
                  ...
                  onClick={() => navigate(`/reviews/file/${file._id}`)}
                  sx={{
                    background: 'linear-gradient(45deg, #e91e63, #ff5722)',
                    color: 'white',
                    fontWeight: 'bold',
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          <Typography>
            ඔබට "{selectedFile?.title}" ගොනුව ඉවත් කිරීමට අවශ්‍යද? මෙම ක්‍රියාව ආපසු හැරවිය නොහැක.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ p: 3 }}>
...
export default ReviewsFolderView;
